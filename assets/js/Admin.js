...
window.addEventListener('load', async () => {
	...
	const response = await fetch(
		`${url}?action=unofficial_convertkit_info`
	);
	...
	if (false === account) {
		document.getElementById('uck-account-status-text').innerText = __('No information about your account.', 'unofficial-convertkit');
		return;
	}
	...
	document.getElementById('uck-account-email').innerText = account.primary_email_address ?? '';
});
